function [fitparams,betas, modelfit] = main_model_EU(data,model,dat_class,num_it,tau)
...
for start=1:num_it
    ...
    if strcmp(model,'multi')
        ub=[60 3];
        x0 = [ub(1)*rand(1) 3*rand(1) ];
        
    elseif strcmp(model,'adi') || strcmp(model,'adi_test')
        ub=[60 60 3];
        x0 = [ub(1)*rand(1) ub(2)*rand(1) 3*rand(1) ];
        
    elseif strcmp(model,'hybrid') || strcmp(model,'aditau')
        ub=[60 60 60 3];
        x0 = [ub(1)*rand(1) ub(2)*rand(1) ub(3)*rand(1) 3*rand(1) ];
    ...
    [fitparams_between, modelfit_between] = fmincon(@mod_mag, x0, [], [], [], [], lb,ub,[],options);
...
elseif strcmp(model,'adi')|| strcmp(model,'aditau')|| strcmp(model,'adi_test')
...
% ================ PT model ========================= %
    function energy = mod_mag(params)
        ...
        elseif strcmp(model,'notau_multi')
