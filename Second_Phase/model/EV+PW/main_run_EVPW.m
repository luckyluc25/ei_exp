...
addpath C:\Users\lucak\Desktop\clash_final\Second_Phase\Modelling_lokal\EV+PW_ourdistort
...
for model_idx=1:numel(model)
    ...
    for iteration=1:length(vp)
        ...
        if strcmp(model{model_idx},'fixtau_multi')
            parameter=importdata('C:\Users\lucak\Desktop\clash_final\Second_Phase\Modelling_lokal\extended_fitting\EVPW_ourdistortmultilu.mat');
        ...
        [params, betas,fitval] = main_model_EVPW(org_data,model{model_idx},'lu',num_it,tau);
        bic = 2*fitval + numel(params)*log(size(data,1));
        aic=2*fitval + (2*numel(params));
        cp_EVPW_ourdistort.params(iteration,:) = [params, betas,fitval, bic,aic];
        
    end
    
    cd 'C:\Users\lucak\Desktop\clash_final\Second_Phase\Modelling_lokal\extended_fitting'
    
    save(['EVPW_ourdistort' model{model_idx} 'lu'], 'cp_EVPW_ourdistort');
    
    clear cp_EVPW_ourdistort fitval bic aic
    
    %% Do the same for NatNeuro 2012 data
    ...
    for iteration=1:length(vp)
        ...
        if strcmp(model{model_idx},'fixtau_multi')
            parameter=importdata('C:\Users\lucak\Desktop\clash_final\Second_Phase\Modelling_lokal\extended_fitting\EVPW_ourdistortmultiger.mat');
            parameter=parameter.params;
            med=median(parameter);
            tau=med(1);
        else
        ...
        % call model estimation
        [fitparams,betas, modelfit] = main_model_EVPW(data,model{model_idx},'ger',num_it,tau);
        bic = 2*modelfit + numel(fitparams)*log(size(data,1));
        aic=2*modelfit + (2*numel(fitparams));
        cp_EVPW_ourdistort.params(iteration,:) = [fitparams, betas,modelfit,bic,aic];
        
    end
    
    cd   'C:\Users\lucak\Desktop\clash_final\Second_Phase\Modelling_lokal\extended_fitting'
    save(['EVPW_ourdistort' model{model_idx} 'ger'], 'cp_EVPW_ourdistort');
