...
for iteration=1:numel(vp)
    ...
    nb_left= (rp(:,1)>=rp(:,2) & rm(:,1)>=rm(:,2));
    nb_right= (rp(:,2)>=rp(:,1) & rm(:,2)>=rm(:,1));
    trial_count=[1:length(nb_obj)]';
    
    
    
    x=zscore([(rp(:,2)-rp(:,1)) rm(:,2)-rm(:,1) ev(:,1)+ev(:,2) costs (pvd') repchoices(:,1) all_wins(:,1) choices_pvd nb_obj trial_count sw]);
    ...
    logRes_pvd_prob_mag(iteration,:) = (c);
    
    clear x y rm rp
end

% save and plot
save( 'C:\Users\lucak\Desktop\clash_script_upload\Second_Phase\DV\logRes_pvd_prob_mag','logRes_pvd_prob_mag');


[x y z stats]=ttest(logRes_pvd_prob_mag)
check=mes(logRes_pvd_prob_mag,0,'U3_1')


% plot effects of val diff on choice
model_series_short = mean(logRes_pvd_prob_mag(:,y<0.05));
model_error_short = std(logRes_pvd_prob_mag(:,y<0.05))./sqrt(size(logRes_pvd_prob_mag(:,y<0.05),1));
para={'Bias','prob_diff','mag_diff','PVD','Costs','PVD'};
